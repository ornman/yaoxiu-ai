...
当遇到以下情况时，**必须**调用 Agent 工具并行处理：

1. **多文件修改任务** - 同时修改 3+ 个文件
2. **复杂功能开发** - 涉及多个模块的新功能
3. **批量重构** - 代码结构调整影响多个地方
4. **紧急修复** - 需要同时进行问题定位和修复
...
// Agent 1: 处理前端样式
Agent({
    description: "修改用户界面样式",
    prompt: "修改 index.html 中的用户消息气泡样式..."
})

// Agent 2: 处理后端逻辑
Agent({
    description: "优化 API 接口",
    prompt: "修改 worker.js 中的消息处理逻辑..."
})

// Agent 3: 更新文档
Agent({
    description: "更新项目文档",
    prompt: "修改 AGENTS.md 和 PRD 文档..."
})
```
...
| 前端 + 后端 | 分别分配给不同 Agent |
| 多个独立文件 | 按文件类型分组 |
| 重构 + 新功能 | 重构一个 Agent，新功能一个 Agent |
| Bug 修复 | 定位一个 Agent，修复一个 Agent |
